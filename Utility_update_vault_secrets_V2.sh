...
update_key_value()
   {
        ...
        read -p 'Enter a secret key name for which you want to update the value: ' secret_key_name
		  # echo " Current value of the entered key is below: "
		# is_key_present= $(grep -w $secret_key_name $secret_storage_path$host_group_name$secret_storage_file_name)
	    	grep -w $secret_key_name $secret_storage_path$host_group_name$secret_storage_file_name
                 is_key_present= $(grep -w $secret_key_name $secret_storage_path$host_group_name$secret_storage_file_name)

		 echo "is_key_present curr Key value: " $is_key_present
		 echo
